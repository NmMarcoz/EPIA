...
const Acess: React.FC<AccessProps> = ({ onLoginSuccess }) => {
  ...
  // Inicializar webcam para modo operário
//   useEffect(() => {
...
//         setScanningStatus("Aproxime seu cartão ou QR Code da câmera")
//       }
//     } catch (error) {
//       console.error("Erro ao acessar webcam:", error)
//       setScanningStatus("Erro ao acessar câmera")
//     }
//   }
...
//   }
  ...
  const handleOperatorAccess = () => {
    ...
    // Simulação de leitura de QR/RFID - substituir por integração real
    setTimeout(() => {
      const operatorUser: User = {
        id: "OP_" + Date.now(),
        name: "Operário",
        type: "operator",
        cardId: "CARD_" + Math.random().toString(36).substr(2, 9).toUpperCase(),
      }

      localStorage.setItem("epiaUser", JSON.stringify(operatorUser))
      localStorage.setItem("epiaAuthToken", "operator_token_" + Date.now())

      onLoginSuccess(operatorUser)
      setIsLoading(false)
    }, 2000)
  }
        ...
        <div className="content-card">
          {authMode === "admin" ? (
            ...
            <div className="operator-interface">
              ...
              <button onClick={handleOperatorAccess} disabled={isLoading} className="auth-button operator-button">
